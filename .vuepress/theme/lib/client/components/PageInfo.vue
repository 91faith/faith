<template>
  <div v-if="showPageInfo" class="page-info">
    <Xicons v-if="!!author" class="icon iconfont icon-yonghu" :text="author" />

    <Xicons v-if="!!date" class="icon iconfont icon-shijian" :text="date" />

    <Xicons v-if="!!tagVal" class="icon iconfont icon-24gf-tags2" :text="tagVal" @click="taglink?taglinkRouter(taglink):''"/>
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const tagVal = computed(
      () => pageData?.value?.frontmatter?.tagVal || themeData.value.tagVal || ''
    )
    const taglink = computed(
      () => pageData?.value?.frontmatter?.taglink || themeData.value.taglink || ''
    )
    ...
    function taglinkRouter(taglink){
      console.log(taglink,this)
      this.$router.push({path:taglink})
    }
    return {
      author,
      tagVal,
